...
export default class SnowScene {
  ...
  snowflakeTextureLoaded = (texture) => {
    ...
    // itemSize = 3 because there are 3 values (components) per vertex
    this.flakesGeometry.addAttribute( 'position', new THREE.BufferAttribute( this.vertices, 3 ) );
    this.flakesGeometry.addAttribute( 'actualYInCaseInvisible', new THREE.BufferAttribute( this.actualYInCaseInvisible, 1 ) );
    this.flakeMaterial = new THREE.ParticleBasicMaterial({ map: this.snowflaketexture, transparent: true, size: 5 });
  ...
  createCamera = () => {
    const camera = new THREE.PerspectiveCamera(
      Constants.viewAngle,
      Constants.width / Constants.height, Constants.near, Constants.far
    );

    // position 0,0,0 is lower left corner of picture (the floor below the middle of the tv).
    camera.position.set(Constants.wallWidth / 2, Constants.cameraHeight, Constants.cameraToBackWall);
    camera.lookAt(new THREE.Vector3(Constants.wallWidth / 2, Constants.daddyFaceHeight, Constants.daddyFaceToBackWall));
    return camera;
  }
  ...
  createFloor = () => {
    ...
    const floorGeometry = new THREE.PlaneGeometry(Constants.wallWidth, Constants.backWallToWhereFloorIsVisible);
  ...
  createUmbrella = () => {
    const umbrellaMaterial = new THREE.MeshBasicMaterial( { color: 0x777777, side: THREE.DoubleSide } );
    umbrellaMaterial.transparent = true;
    umbrellaMaterial.opacity = 0.5;
    const umbrellaGeometry = new THREE.SphereGeometry(85, 8, 6, 0, 6.283, 0, 0.8);
  ...
  removeUmbrella = (removedFromActualScene) => {
    ...
    for (let xCoordinate = 0; xCoordinate < Constants.wallWidth; xCoordinate++ ) {
      for (let zCoordinate = 0; zCoordinate < Constants.backWallToWhereFloorIsVisible; zCoordinate++ ) {
        const index = xCoordinate * Constants.backWallToWhereFloorIsVisible + zCoordinate;
        if (this.snowflakeLandingSpots[index] > Constants.daddyFaceHeight + 10) {
  ...
  recalculateSnowflakeLandingSpots = () => {
    ...
    for (let xCoordinate = 0; xCoordinate < Constants.wallWidth; xCoordinate++ ) {
      for (let zCoordinate = 0; zCoordinate < Constants.backWallToWhereFloorIsVisible; zCoordinate++ ) {
        const index = xCoordinate * Constants.backWallToWhereFloorIsVisible + zCoordinate;
        const origin = new THREE.Vector3(xCoordinate, Constants.topOfWreath, zCoordinate);
        const direction = new THREE.Vector3(0, -1, 0);
        const raycaster = new THREE.Raycaster(origin, direction, 20, Constants.topOfWreath);
        ...
        if (intersects.length > 0) {
          ...
          this.snowflakeLandingSpots[index] = Math.max(this.snowflakeLandingSpots[index], (Constants.topOfWreath - intersects[0].distance));
        } else {
          this.snowflakeLandingSpots[index] = Math.max(this.snowflakeLandingSpots[index], 0);
        }
      }
    }
  }

  addHeadAndShoulders = (scene, radius, xPosition, yPosition, zPosition,
    ...
    const rightShoulderGeometry = new THREE.CylinderGeometry(radius * 0.7, radius * 0.7, lengthOfShoulder);
    const rightShoulder = new THREE.Mesh(rightShoulderGeometry, material);
    rightShoulder.rotation.z = Math.PI * (0.3 + rightShoulderRotationAdjustment);
    rightShoulder.position.x = head.position.x + (lengthOfShoulder / 2);
    rightShoulder.position.y = head.position.y - (radius * 2) + rightShoulderHeightAdjustment;
    rightShoulder.position.z = head.position.z;
    rightShoulder.updateMatrixWorld();
    scene.add(rightShoulder);

    const leftShoulderGeometry = new THREE.CylinderGeometry(radius * 0.7, radius * 0.7, lengthOfShoulder);
    ...
    leftShoulder.position.y = head.position.y - (radius * 2) + leftShoulderHeightAdjustment;
    ...
    if (whichPerson === 'daddy') {
      ...
      const daddyLowerLegGeometry = new THREE.CylinderGeometry(daddyLowerLegRadius, daddyLowerLegRadius, daddyLowerLegLength);
      ...
      const daddyRightLowerLeg = new THREE.Mesh(daddyLowerLegGeometry, material);
    ...
    if (whichPerson === 'mommy') {
      ...
      const mommyUpperArmGeometry = new THREE.CylinderGeometry(mommyUpperArmRadius, mommyUpperArmRadius, shoulderToElbowLength);
      ...
      const mommyForearmGeometry = new THREE.CylinderGeometry(mommyForearmRadius, mommyForearmRadius, mommyForearmLength);
    ...
    if (whichPerson === 'lily') {
      ...
      const lilyTongueGeometry = new THREE.CylinderGeometry(lilyTongueRadius, lilyTongueRadius, lilyTongueLength);
    ...
    if (whichPerson === 'willow') {
      const tongueMaterial = new THREE.MeshBasicMaterial( { color: 0x77ff77, side: THREE.DoubleSide } );
      tongueMaterial.transparent = true;
      tongueMaterial.opacity = 0.5;
      const willowTongueLength = 4;
      const willowTongueRadius = 2;
      const willowTongueGeometry = new THREE.CylinderGeometry(willowTongueRadius, willowTongueRadius, willowTongueLength);
  ...
  addMiscellaneousObjects = (scene) => {
    ...
    const reindeerTorsoGeometry = new THREE.CylinderGeometry(leftReindeerTorsoRadius, leftReindeerTorsoRadius, leftReindeerTorsoLength);
    ...
    const reindeerHeadGeometry = new THREE.CylinderGeometry(leftReindeerHeadRadius, leftReindeerHeadRadius, leftReindeerHeadLength);
    ...
    const rightReindeerTorsoGeometry = new THREE.CylinderGeometry(1, leftReindeerTorsoRadius, leftReindeerTorsoLength - 7);
    const rightReindeerTorso = new THREE.Mesh(rightReindeerTorsoGeometry, material);
    ...
    const footRestGeometry = new THREE.CylinderGeometry(footRestRadius, footRestRadius, footRestHeight);
  ...
  addHelperSceneAndObjects = (showOnActualScene) => {
    ...
    if (showOnActualScene) {
      this.addHeadAndShoulders(this.scene, 9, Constants.wallWidth / 2 + 3, Constants.daddyFaceHeight, Constants.daddyFaceToBackWall, 0, 0, 0, 0, 'daddy');
      this.addHeadAndShoulders(this.scene, 9, Constants.wallWidth / 2 - 28, Constants.daddyFaceHeight - 1, Constants.daddyFaceToBackWall, 6, -0.18, 0, 0, 'mommy');
      this.addHeadAndShoulders(this.scene, 7, Constants.wallWidth / 2 - 24, Constants.daddyFaceHeight - 22, Constants.daddyFaceToBackWall + 10, -3, 0, 0, 0, 'willow');
      this.addHeadAndShoulders(this.scene, 7, Constants.wallWidth / 2 + 42, Constants.daddyFaceHeight - 17, Constants.daddyFaceToBackWall + 5, 0, 0, -2, -0.15, 'lily');
      this.addMiscellaneousObjects(this.scene);
    }
    this.addHeadAndShoulders(this.helperScene, 9, Constants.wallWidth / 2 + 3, Constants.daddyFaceHeight, Constants.daddyFaceToBackWall, 0, 0, 0, 0, 'daddy');
    this.addHeadAndShoulders(this.helperScene, 9, Constants.wallWidth / 2 - 28, Constants.daddyFaceHeight - 1, Constants.daddyFaceToBackWall, 6, -0.18, 0, 0, 'mommy');
    this.addHeadAndShoulders(this.helperScene, 7, Constants.wallWidth / 2 - 24, Constants.daddyFaceHeight - 22, Constants.daddyFaceToBackWall + 10, -3, 0, 0, 0, 'willow');
    this.addHeadAndShoulders(this.helperScene, 7, Constants.wallWidth / 2 + 42, Constants.daddyFaceHeight - 17, Constants.daddyFaceToBackWall + 5, 0, 0, -2, -0.15, 'lily');
    ...
    this.snowflakeLandingSpots = new Float32Array(Constants.wallWidth * Constants.backWallToWhereFloorIsVisible);
  ...
  snowflakeCanFall = (vertexX, vertexY, vertexZ) => {
    ...
    const index = (xCoord, zCoord) => xCoord * Constants.backWallToWhereFloorIsVisible + zCoord;
    return this.snowflakeLandingSpots[index(vertexX, vertexZ)] < vertexY - 2
      && (vertexX === 0 || this.snowflakeLandingSpots[index(vertexX - 1, vertexZ)] < vertexY - 1)
      && (vertexX === Constants.wallWidth || this.snowflakeLandingSpots[index(vertexX + 1, vertexZ)] < vertexY - 1)
      && (vertexZ === 0 || this.snowflakeLandingSpots[index(vertexX, vertexZ - 1)] < vertexY - 1)
      && (vertexZ === Constants.backWallToWhereFloorIsVisible || this.snowflakeLandingSpots[index(vertexX, vertexZ + 1)] < vertexY - 1);
  }

  createLaps = () => {
    const lapMaterial = new THREE.MeshBasicMaterial( { color: 0x777777, side: THREE.DoubleSide } );
  ...
  snowflakeIsVisible = () => { // (vertexX, vertexY, vertexZ) => {
    return true;
  ...
  updateSnowflakeLandingSpotsWithSnowflakeLanded = (vertexX, vertexY, vertexZ) => {
    const index = (xCoord, zCoord) => xCoord * Constants.backWallToWhereFloorIsVisible + zCoord;
    ...
    switch (action.type) {
      ...
      case 'ADVANCE_A_FRAME':
        switch (state.background) {
          ...
          case Backgrounds.indoorGearConfused:
            if (state.frameNumber - state.frameNumberOfLastBackgroundChange
              > (Constants.secondsFromConfusedToShivering * Constants.framesPerSecond)
            ) {
          ...
          case Backgrounds.indoorGearShivering:
            if (state.frameNumber - state.frameNumberOfLastBackgroundChange
              > (Constants.secondsFromShiveringToWinterGear * Constants.framesPerSecond)
            ) {
          ...
          case Backgrounds.winterGearSmiling:
            if (state.frameNumber - state.frameNumberOfLastBackgroundChange
              > (Constants.secondsFromWinterGearToUmbrella * Constants.framesPerSecond)
            ) {
              this.setBackground(state, Backgrounds.winterGearWithUmbrella);
              this.addUmbrella(Constants.helpFigureOutWhereThingsAre);
            }
            if (state.snowflakesPerSecond < Constants.heavySnowflakesPerSecond) {
          ...
          case Backgrounds.winterGearWithUmbrella:
            if (state.frameNumber - state.frameNumberOfLastBackgroundChange
              > (Constants.secondsBeforeUmbrellaDisappears * Constants.framesPerSecond)
            ) {
              this.setBackground(state, Backgrounds.winterGearAnticipatingSnowDump);
              this.removeUmbrella(Constants.helpFigureOutWhereThingsAre);
            }
            break;
          case Backgrounds.winterGearAnticipatingSnowDump:
            if (state.frameNumber - state.frameNumberOfLastBackgroundChange
              > (Constants.secondsOfSnowDumpAnticipation * Constants.framesPerSecond)
            ) {
